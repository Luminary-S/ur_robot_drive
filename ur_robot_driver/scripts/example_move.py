...
def test_ori_ctr_old():
    ...
    try:
        ...
        trace_err("FT KalmanFilter thread error")
        return 
    # if not alive: 
    ...
    res1 = urm.go_to_init_upinstall()
    sleep(1)
    # F0 = [2.1017, 1.0228, 1.7373, -0.0643, 0.004, -0.0433]
    # F_d = [-8.5215, 14.041, -33.782, -0.9937, 0.797, 0.5795]# measured desired, xyz are inverse while MxMyMz are right
    # F = ft.f.get_latest()
    F_0 = ft.f.get_filtered_init()
    F_d = [F_0[0], F_0[1], F_0[2], F_0[3]-5, F_0[4], F_0[5] ]
    delta_F_tmp = np.array(F_d) - np.array(F_0)

    delta_F = force_measure_to_in_force_frame(delta_F_tmp) 

    HBctr = HBCTR()
    Smat,Cmat = HBctr.get_stiffness_matrix()
    # get the relative speed in FT frame
    F_Threshold = 10 * np.array([0.1,0.1,0.1,0.001,0.001,0.001])
    abs_delta_F = np.abs(delta_F) 
    loop = 0
    HZ = urm.frequency
    ddF_pub, dF_pub, ee_in_ee_TW_pub, ee_in_base_TW_pub = exp_define_pub()
    while not rospy.is_shutdown() and np.sum( abs_delta_F < F_Threshold ) < 6: #and loop <100:
        trace_loop(loop)
        try:
            if loop > 0 :
                F_0 = ft.f.get_filtered_latest()
            delta_delta_F = np.array(F_d) - np.array(F_0) - delta_F_tmp
            delta_F_tmp = np.array(F_d) - np.array(F_0)
            delta_F = force_measure_to_in_force_frame(delta_F_tmp)
            abs_delta_F = np.abs(delta_F)
            trace_round("F_d", F_d, "F_0", F_0, "delta_F",delta_F, "delta_delta_F", delta_delta_F)
            delta_X_in_FT = HBctr.CTR_desired_FT(delta_F, Cmat)
            trace_round("delta_X_in_FT", delta_X_in_FT)
            delta_X_in_base = urm.speedl_tool(delta_X_in_FT, 1, 1.0/HZ)

            exp_publish(ddF_pub,delta_delta_F, dF_pub, delta_F_tmp, ee_in_base_TW_pub, delta_X_in_base, ee_in_ee_TW_pub, delta_X_in_FT)
            loop +=1
            urm.rate.sleep()
        except KeyboardInterrupt:
            rospy.signal_shutdown("KeyboardInterrupt")
